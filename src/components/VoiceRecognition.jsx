...
export const VoiceRecognition = forwardRef(({ onTranscript, onAutoSend, isActive, onToggle, language = 'kk' }, ref) => {
    ...
    // Calculate silence countdown for auto-send
    const [silenceCountdown, setSilenceCountdown] = useState(0);

    useEffect(() => {
        if (isListening && lastSpeechTime && autoSendEnabled) {
            const interval = setInterval(() => {
                const timeSinceLastSpeech = Date.now() - lastSpeechTime;
                const remainingTime = Math.max(0, 2000 - timeSinceLastSpeech);
    ...
    }, [isListening, lastSpeechTime, autoSendEnabled]);
                ...
                {(interimTranscript || transcript) && (
                    <div className="mt-1 max-w-48">
                        <div className="text-xs text-gray-300 bg-gray-800/80 rounded px-2 py-1 backdrop-blur-sm">
                            ...
                            {/* Auto-send countdown */}
                            {autoSendEnabled && silenceCountdown > 0 && transcript.trim() && (
                                <div className="mt-1 flex items-center space-x-1">
                                    <div className="w-full bg-gray-600 rounded-full h-1">
                                        <div
                                            className="bg-green-400 h-1 rounded-full transition-all duration-100"
                                            style={{ width: `${((2000 - silenceCountdown) / 2000) * 100}%` }}
                                        ></div>
                                    </div>
                                    <span className="text-green-400 text-xs font-mono">
                                        {Math.ceil(silenceCountdown / 1000)}s
                                    </span>
                                </div>
                            )}
                        </div>
